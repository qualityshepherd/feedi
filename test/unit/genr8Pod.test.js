...
test('Podcast: extractAudioSrc returns src from audio tag', t => {
  t.is(extractAudioSrc('<audio src="/assets/ep1.mp3"></audio>'), '/assets/ep1.mp3')
})

test('Podcast: extractAudioSrc returns null when no audio tag', t => {
  t.is(extractAudioSrc('<p>no audio</p>'), null)
})

// ── resolveAudioUrl ───────────────────────────────────────────

test('Podcast: resolveAudioUrl makes relative src absolute', t => {
  t.is(resolveAudioUrl('/assets/ep1.mp3', baseUrl), 'https://example.com/assets/ep1.mp3')
})

test('Podcast: resolveAudioUrl leaves absolute src unchanged', t => {
  t.is(resolveAudioUrl('https://cdn.example.com/ep1.mp3', baseUrl), 'https://cdn.example.com/ep1.mp3')
})

test('Podcast: resolveAudioUrl returns null for null src', t => {
  t.is(resolveAudioUrl(null, baseUrl), null)
})

// ── isRfc2822Date ─────────────────────────────────────────────

test('Podcast: isRfc2822Date accepts valid GMT date', t => {
  t.ok(isRfc2822Date('Wed, 01 Jan 2025 00:00:00 GMT'))
})

test('Podcast: isRfc2822Date accepts timezone offset', t => {
  t.ok(isRfc2822Date('Wed, 06 Jul 2014 13:00:00 -0700'))
})

test('Podcast: isRfc2822Date rejects US date format', t => {
  t.falsy(isRfc2822Date('7/6/2014 1:00:00 PM'))
})

test('Podcast: isRfc2822Date rejects ISO 8601', t => {
  t.falsy(isRfc2822Date('2025-01-01T00:00:00Z'))
})

test('Podcast: new Date().toUTCString() passes isRfc2822Date', t => {
  t.ok(isRfc2822Date(new Date('2025-01-01').toUTCString()))
})

// ── buildPodItem ──────────────────────────────────────────────

test('Podcast: buildPodItem returns null when no audio', t => {
  t.is(buildPodItem({ ...fakePodcast, html: '<p>no audio</p>' }, baseUrl), null)
})

test('Podcast: buildPodItem uses /posts/slug not hash route', t => {
  const item = buildPodItem(fakePodcast, baseUrl)
  t.ok(item.includes('/posts/episode-1'))
  t.falsy(item.includes('#post'))
})

test('Podcast: buildPodItem enclosure has absolute url', t => {
  const item = buildPodItem(fakePodcast, baseUrl)
  t.ok(item.includes('url="https://'))
})

test('Podcast: buildPodItem enclosure has audio/mpeg type', t => {
  t.ok(buildPodItem(fakePodcast, baseUrl).includes('type="audio/mpeg"'))
})

test('Podcast: buildPodItem enclosure uses provided length', t => {
  const item = buildPodItem(fakePodcast, baseUrl, 5650889)
  t.ok(item.includes('length="5650889"'))
})

test('Podcast: buildPodItem pubDate is RFC 2822', t => {
  const item = buildPodItem(fakePodcast, baseUrl)
...
test('Podcast: validatePodFeed returns no errors for valid feed', t => {
  t.deepEqual(validatePodFeed(buildPodFeed([fakePodcast], fakeCfg)), [])
})

test('Podcast: validatePodFeed catches missing itunes namespace', t => {
...
test('Podcast: validatePodFeed catches missing content namespace', t => {
...
test('Podcast: validatePodFeed catches missing itunes:image', t => {
...
test('Podcast: validatePodFeed catches missing itunes:category', t => {
...
test('Podcast: validatePodFeed catches missing language', t => {
...
test('Podcast: validatePodFeed catches missing item enclosure', t => {
  const feed = buildPodFeed([fakePodcast], fakeCfg).replace(/<enclosure.*?\/>/g, '')
  t.ok(validatePodFeed(feed).some(e => e.includes('enclosure')))
})

test('Podcast: validatePodFeed catches relative enclosure url', t => {
